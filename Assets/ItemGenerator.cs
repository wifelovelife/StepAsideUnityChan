...
public class ItemGenerator : MonoBehaviour {
    public GameObject carPrefab; // carPrefabオブジェクトを読み込む変数carprefabを公開
    public GameObject coinPrefab; // coinPrefabオブジェクトを読み込む変数coinPrefabを公開
    public GameObject conePrefab; // conePrefabオブジェクトを読み込む変数conePrefabを公開
            ...
            if (num <= 2) { // アイテム発生率を設定（20％以下）
                ...
                for (int j = -1; j <= 1; j++) {
                    ...
                    if (1 <= item && item <= 6) { // 60%配置
                        ...
                        car.transform.position = new Vector3(posRange * j, car.transform.position.y, i + offsetZ); // carPrefabをランダムに生成
                    }
                }
            }
        }
